...
```bash
sudo pacman -U btrfs-backup-3.1.0-1-x86_64.pkg.tar.zst
```
...
```bash
sudo pacman -U btrfs-backup-kde-3.1.0-1-x86_64.pkg.tar.zst
```
...
For a profile upgraded from 3.0, preview and apply the legacy crypttab import:
...
removed in 4.0. Fully unattended operation requires a root-owned key file with
