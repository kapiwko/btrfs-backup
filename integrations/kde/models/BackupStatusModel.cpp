...
QString manager_error_message(const QDBusError& error) {
    ...
    if (name.endsWith(QStringLiteral(".InvalidRequest"))) return i18nd(translation_domain, "The request is invalid.");
    if (name.endsWith(QStringLiteral(".SourceMissing"))) return i18nd(translation_domain, "The selected backup source does not exist.");
    if (name.endsWith(QStringLiteral(".SourceNotSubvolume"))) return i18nd(translation_domain, "The selected backup source is not a Btrfs subvolume.");
    if (name.endsWith(QStringLiteral(".SourceUnavailable"))) return i18nd(translation_domain, "The selected backup source cannot be inspected.");
    if (name.endsWith(QStringLiteral(".NotFound"))) return i18nd(translation_domain, "The requested item was not found.");
    if (name.endsWith(QStringLiteral(".NotAuthorized"))) return i18nd(translation_domain, "The operation was cancelled or you do not have permission to perform it.");
    if (name.endsWith(QStringLiteral(".Busy"))) return i18nd(translation_domain, "The requested item is busy.");
    if (name.endsWith(QStringLiteral(".RunMismatch"))) return i18nd(translation_domain, "The active backup run has changed. Refresh and try again.");
    if (name.endsWith(QStringLiteral(".TargetUnavailable"))) return i18nd(translation_domain, "The backup device is disconnected or unavailable.");
    if (name.endsWith(QStringLiteral(".Conflict"))) return i18nd(translation_domain, "The operation conflicts with the current state. Refresh and try again.");
    if (name.endsWith(QStringLiteral(".SaveFailed"))) return i18nd(translation_domain, "The configuration could not be saved.");
    if (name.endsWith(QStringLiteral(".RollbackIncomplete"))) return i18nd(translation_domain, "The operation could not be fully rolled back. Review the system log.");
...
QString BackupStatusModel::profile() const { return profile_; }
...
void BackupStatusModel::setProfile(const QString& profile) {
    ...
    run_.reset();
    target_.reset();
    if (active_) history_.setProfileId(profile_); else history_.reset();
    ...
    if (active_) {
        syncFromDirectory();
        if (manager_connected_) { requestStatus(); requestDeviceState(); requestHistory(); }
    }
}

bool BackupStatusModel::managerConnected() const { return manager_connected_; }
QString BackupStatusModel::profileName() const { return profile_name_; }
bool BackupStatusModel::profileEnabled() const { return profile_enabled_; }
bool BackupStatusModel::configurationValid() const { return configuration_valid_; }
QString BackupStatusModel::configurationErrorCode() const { return configuration_error_code_; }
RunStatusModel* BackupStatusModel::run() { return &run_; }
TargetStatusModel* BackupStatusModel::target() { return &target_; }
BackupHistoryModel* BackupStatusModel::history() { return &history_; }
bool BackupStatusModel::operationPending() const { return operation_pending_; }
QString BackupStatusModel::lastOperation() const { return last_operation_; }
QString BackupStatusModel::lastError() const { return last_error_; }
QString BackupStatusModel::lastErrorCode() const { return last_error_code_; }
...
int BackupStatusModel::historyLimit() const { return history_limit_; }

void BackupStatusModel::setHistoryLimit(int limit) {
    const int bounded = std::clamp(limit, 1, 10);
    if (history_limit_ == bounded) return;
    history_limit_ = bounded;
    history_.setPageSize(history_limit_);
    emit historyLimitChanged();
}

ProfileDirectoryModel* BackupStatusModel::directory() const { return directory_; }
...
void BackupStatusModel::setDirectory(ProfileDirectoryModel* directory) {
    ...
    if (directory_ == replacement) return;
    if (directory_ != nullptr) disconnect(directory_, nullptr, this, nullptr);
    ...
    if (active_) {
        if (directory_ == &local_directory_) local_directory_.start();
        syncFromDirectory();
    }
}
...
void BackupStatusModel::stop() {
    ...
    operation_message_timer_.stop();
    last_operation_.clear();
    if (directory_ == &local_directory_) local_directory_.stop();
    emit operationChanged();
    setManagerConnected(false);
}

void BackupStatusModel::refreshNow() {
    if (!active_) { start(); return; }
    if (directory_ != nullptr) directory_->refreshNow();
    if (manager_connected_) { requestStatus(); requestDeviceState(); requestHistory(); }
}

void BackupStatusModel::startBackup() { requestOperation(QLatin1String(btrfsbackup::manager_protocol::method::start_backup), {profile_}); }
void BackupStatusModel::validateTarget() { requestOperation(QLatin1String(btrfsbackup::manager_protocol::method::validate_target), {profile_}); }
void BackupStatusModel::ejectTarget() { requestOperation(QLatin1String(btrfsbackup::manager_protocol::method::eject_target), {profile_}); }
...
void BackupStatusModel::connectDirectory() {
    ...
    connect(directory_, &ProfileDirectoryModel::profileStatusInvalidated, this, [this](const QString& profile_id) {
        if (active_ && manager_connected_ && profile_id == profile_) requestStatus();
    });
    connect(directory_, &ProfileDirectoryModel::profileHistoryInvalidated, this, [this](const QString& profile_id) {
        if (active_ && manager_connected_ && profile_id == profile_) { requestStatus(); requestHistory(); }
    });
    connect(directory_, &ProfileDirectoryModel::profileDeviceStateInvalidated, this, [this](const QString& profile_id) {
        if (active_ && manager_connected_ && profile_id == profile_) requestDeviceState();
    });
    connect(directory_, &ProfileDirectoryModel::refreshRequested, this, [this]() {
        if (active_ && manager_connected_) { requestStatus(); requestDeviceState(); requestHistory(); }
    });
}

void BackupStatusModel::syncFromDirectory() {
    if (!active_ || directory_ == nullptr) return;
    if (!directory_->managerConnected()) {
        managerUnavailable();
        if (!directory_->lastError().isEmpty()) setLastError(directory_->lastError(), directory_->lastErrorCode());
        return;
    }
    ...
    for (const QVariant& value : directory_->profiles()) {
        ...
        if (decoded.value(QStringLiteral("profileId")).toString() != profile_) continue;
    ...
    if (became_connected) { requestStatus(); requestDeviceState(); requestHistory(); }
}

void BackupStatusModel::requestDeviceState() {
    if (directory_ == nullptr || !directory_->supports(QLatin1String(btrfsbackup::manager_protocol::feature::device_state))) return;
    if (device_request_pending_) { device_refresh_queued_ = true; return; }
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, watcher, request_generation, requested_profile](QDBusPendingCallWatcher*) {
        ...
        watcher->deleteLater();
        if (!active_ || request_generation != generation_ || requested_profile != profile_) return;
        device_request_pending_ = false;
        const bool refresh_again = std::exchange(device_refresh_queued_, false);
        if (reply.isError()) setLastError(tr("Could not load backup target state from the system manager."), reply.error().name());
        else applyDeviceState(reply.value());
        if (refresh_again) requestDeviceState();
    });
}

void BackupStatusModel::requestHistory() { history_.loadFirstPage(); }

void BackupStatusModel::requestStatus() {
    if (status_request_pending_) { status_refresh_queued_ = true; return; }
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, watcher, request_generation, requested_profile](QDBusPendingCallWatcher*) {
        ...
        if (reply.isError()) setLastError(tr("Could not load backup status from the system manager."), reply.error().name());
        else applyStatus(reply.value());
        if (refresh_again) requestStatus();
    });
}
...
void BackupStatusModel::requestOperation(const QString& method, const QVariantList& arguments) {
    if (!active_ || !manager_connected_ || operation_pending_ || directory_ == nullptr) return;
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this, watcher, request_generation](QDBusPendingCallWatcher*) {
        const QDBusPendingReply<QString> reply = *watcher;
        watcher->deleteLater();
        if (!active_ || request_generation != generation_) return;
        operation_pending_ = false;
        if (reply.isError()) {
...
void BackupStatusModel::setManagerConnected(bool connected) {
    if (manager_connected_ == connected) return;
...
void BackupStatusModel::setLastError(const QString& message, const QString& code) {
    if (last_error_ == message && last_error_code_ == code) return;
    last_error_ = message;
...
void BackupStatusModel::managerUnavailable() {
    ...
    emit operationChanged();
    setManagerConnected(false);
    if (last_error_.isEmpty()) setLastError(tr("The system backup manager is unavailable."), QStringLiteral("manager.unavailable"));
}
