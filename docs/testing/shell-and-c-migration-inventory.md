...
| CMake `install-local` target | Builds and installs both local Arch packages with stage diagnostics. | Build as user; pacman and system-service operations require elevation and are **security-sensitive**. | CMake target and tested command manifest; package hooks remain authoritative. | Migrated in stage 16. |
| `tools/build-release.sh` | Deterministic source/archive creation, CMake native payload, install staging, Arch/Deb/RPM/Nix/Gentoo packaging and package verification. | Release user plus optional container/package tools; package script execution is **security-sensitive**. | Replaced by CPack/CMake install plus modular Python release orchestration in stage 17. | Removed. |
