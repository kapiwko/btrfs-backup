...
Current decision: **NO-GO** until the last 0.3.x bridge release ships the ALPM
pre-transaction migration hook and the three real upgrade paths pass. The 1.0
...
- a real Plasma 6 Wayland smoke run loaded the current plasmoid and its QML
  ...
  inspected;
- a real Arch upgrade from `v0.3.3` without profiles succeeds; with a legacy
  profile pacman reports the preflight scriptlet failure but still installs
  1.0, which is the remaining blocker;
...
`90-btrfs-backup-v4-migration.hook` to the final 0.3.x bridge package. Verify
...
1.0 unless it reveals a regression.
