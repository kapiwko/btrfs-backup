...
void test_creates_directory_through_trusted_parents() {
    ...
    btrfsbackup::ensure_trusted_directory(mount_point, 0755, root, geteuid());
...
void test_rejects_symlink_without_changing_target_permissions() {
    ...
    test_helpers::expect_validation_error("mount symlink rejected", [&] {
        btrfsbackup::ensure_trusted_directory(mount_root / "default", 0755, root, geteuid());
    }, "without symlinks");
...
void test_rejects_writable_parent() {
    ...
    test_helpers::expect_validation_error("writable mount parent rejected", [&] {
        btrfsbackup::ensure_trusted_directory(mount_root / "default", 0755, root, geteuid());
