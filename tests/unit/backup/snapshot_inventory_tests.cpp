...
void test_parse_snapshot_name() {
    std::optional<btrfsbackup::SnapshotName> parsed =
        btrfsbackup::parse_snapshot_name("home-2026-08-23T082504Z", "home");
    ...
    parsed = btrfsbackup::parse_snapshot_name("home-2026-08-23T082504Z-02", "home");
    ...
    test_helpers::expect_true(
        "reject wrong source",
        !btrfsbackup::parse_snapshot_name("root-2026-08-23T082504Z", "home").has_value(),
        "snapshot for another source should be ignored"
    );
    test_helpers::expect_true(
        "reject bad timestamp",
        !btrfsbackup::parse_snapshot_name("home-2026-08-23-082504", "home").has_value(),
        "bad timestamp should be ignored"
    );
    test_helpers::expect_true(
        "reject zero sequence",
        !btrfsbackup::parse_snapshot_name("home-2026-08-23T082504Z-00", "home").has_value(),
...
void test_list_snapshot_inventory() {
    ...
    std::unordered_map<std::string, btrfsbackup::SnapshotMetadata> metadata{
        {
    ...
    std::vector<btrfsbackup::SnapshotInfo> inventory = btrfsbackup::list_snapshot_inventory(
        snapshots,
        "home",
...
void test_empty_and_missing_directories() {
    ...
    std::vector<btrfsbackup::SnapshotInfo> missing = btrfsbackup::list_snapshot_inventory(
        root / "missing",
        "home",
    ...
    std::vector<btrfsbackup::SnapshotInfo> empty = btrfsbackup::list_snapshot_inventory(
        root / "empty",
        "home",
        btrfsbackup::SnapshotSide::Remote,
        [](const fs::path&) -> std::optional<btrfsbackup::SnapshotMetadata> {
...
void test_invalid_inputs() {
    ...
    }, "sourceId contains unsupported characters");

    test_helpers::expect_validation_error("missing metadata reader", [] {
        (void)btrfsbackup::list_snapshot_inventory(
            "/tmp",
            "home",
            btrfsbackup::SnapshotSide::Local,
