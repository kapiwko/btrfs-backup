...
bool array_contains(const btrfsbackup::config::Json& array, const std::string& value) {
    if (!array.is_array()) {
        return false;
    }
    for (const btrfsbackup::config::Json& item : array) {
...
void expect_required(const std::string& name, const btrfsbackup::config::Json& object, const std::string& field) {
    test_helpers::expect_true(name, array_contains(object.at("required"), field), "schema does not require " + field);
}

void expect_no_additional_properties(const std::string& name, const btrfsbackup::config::Json& object) {
...
btrfsbackup::config::Json schema() {
    return btrfsbackup::config::load_json_file(repo_root() / "data" / "schemas" / "profile.schema.json");
}

btrfsbackup::config::Json example_profile() {
    return btrfsbackup::config::load_json_file(repo_root() / "data" / "examples" / "profile.example.json");
}

void test_schema_requires_cpp_required_fields() {
    btrfsbackup::config::Json root = schema();
    ...
    const btrfsbackup::config::Json& target = root.at("properties").at("target");
    ...
    const btrfsbackup::config::Json& paths = root.at("properties").at("paths");
    ...
    const btrfsbackup::config::Json& settings = root.at("properties").at("settings");
    ...
    const btrfsbackup::config::Json& hooks = root.at("properties").at("hooks");
    ...
    const btrfsbackup::config::Json& hook_list = root.at("$defs").at("hookList");
    test_helpers::expect_true("schema hook max", hook_list.at("maxItems") == 64, "hook max should match C++ validator");
    const btrfsbackup::config::Json& hook = hook_list.at("items");
    ...
    const btrfsbackup::config::Json& source = root.at("properties").at("sources").at("items");
...
void test_example_profile_matches_cpp_validator() {
    btrfsbackup::config::Json normalized = btrfsbackup::config::normalize_profile(example_profile());
...
void test_cpp_validator_rejects_schema_rejected_fields() {
    btrfsbackup::config::Json top = example_profile();
    top["unexpected"] = true;
    test_helpers::expect_validation_error("unknown top field", [&] { (void)btrfsbackup::config::normalize_profile(top); }, "profile.unexpected");

    btrfsbackup::config::Json target = example_profile();
    target["target"]["unexpected"] = true;
    test_helpers::expect_validation_error("unknown target field", [&] { (void)btrfsbackup::config::normalize_profile(target); }, "target.unexpected");

    btrfsbackup::config::Json source = example_profile();
    source["sources"].at(0)["unexpected"] = true;
    test_helpers::expect_validation_error("unknown source field", [&] { (void)btrfsbackup::config::normalize_profile(source); }, "sources[0].unexpected");
}
...
void test_schema_and_cpp_require_btrfs_uuid() {
    ...
    btrfsbackup::config::Json profile = example_profile();
    profile["target"].erase("btrfsUuid");
    test_helpers::expect_validation_error("cpp requires btrfs uuid", [&] { (void)btrfsbackup::config::normalize_profile(profile); }, "target.btrfsUuid");
}
