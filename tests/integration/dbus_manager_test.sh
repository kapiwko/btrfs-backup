...
for method in GetCapabilities ListProfiles GetStatus GetHistorySanitized GetDeviceState StartBackup CancelBackup ValidateTarget EjectTarget GetProfileDetails UpdateProfileSettings AddProfileSource UpdateProfileSource RemoveProfileSource DeleteProfile SetProfileEnabled OpenBrowseSession RenewBrowseSession SetBrowseSessionActive CloseBrowseSession ListBrowseDirectory InspectBrowseEntry OpenBrowseFile InspectBrowseRepository ResolveBackupCoverage ListTargetCredentials AddTargetPassphrase AddTargetKey GenerateTargetKey RemoveTargetCredential InspectStorageTopology InspectExistingTarget BuildDevicePreparationPlan ListSourceCandidates StartDevicePreparation GetDevicePreparation CancelDevicePreparation; do
...
set -e
[[ "$devices_status" -eq 0 ]] || fail 'read-only device inventory required authorization'
[[ "$sources_status" -eq 0 ]] || fail 'read-only source discovery required authorization'
