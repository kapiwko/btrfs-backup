...
for incompatible profiles and for installations too old to provide the 3.2.x
migration bridge, preventing a direct package upgrade from discovering the
schema break only after 4.0 is installed.
...
12. version 4.0 does not migrate installed 3.x profiles in place; operators must
...
7. `TODO.md` now records the explicit 4.0 go/no-go decision, locally verified
...
2. the migrator accepts only the legacy options generated by btrfs-backup 3.0,
