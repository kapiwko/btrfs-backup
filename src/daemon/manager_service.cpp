...
btrfsbackup::Json read_bounded_json(const fs::path& path) {
    ...
    try {
        return btrfsbackup::Json::parse(content);
...
btrfsbackup::Json unavailable_status() {
    return {
        {"schemaVersion", 3},
        {"state", "unavailable"},
        {"errorCode", ""},
...
btrfsbackup::Json sanitize_public_status(const btrfsbackup::Json& input) {
    ...
    btrfsbackup::Json result = unavailable_status();
...
btrfsbackup::Json sanitize_private_history(const btrfsbackup::Json& input) {
...
Json ManagerService::get_capabilities() const {
    return {
        {"schemaVersion", 1},
        ...
        {"readOnly", true},
        {"features", Json::array({"profiles", "status", "sanitized-history", "device-state"})},
    };
}

Json ManagerService::list_profiles() const {
    std::error_code error;
    ...
    for (const fs::path& file : files) {
        Json profile = read_bounded_json(file);
        ...
        for (const Json& source : profile.at("sources")) {
        ...
        result.push_back({
            ...
            {"targetName", profile.value("target", Json::object()).value("name", "")},
...
Json ManagerService::get_status(const std::string& profile_id) const {
    validate_profile_id(profile_id);
    ...
    if (regular_file_if_present(last)) {
        Json history = sanitize_private_history(read_bounded_json(last));
        Json result = unavailable_status();
...
Json ManagerService::get_history_sanitized(
...
Json ManagerService::get_device_state(const std::string& profile_id) const {
    validate_profile_id(profile_id);
    const fs::path profile_path = paths_.config_root / "profiles" / profile_id / "profile.json";
    const Profile profile = profile_from_json(read_bounded_json(profile_path), paths_.target_mount_root);
    const fs::path mapper = mapper_path(profile.target.mapper_name, paths_.mapper_root);
    const fs::path mountpoint = paths_.target_mount_root / profile_id;
    const std::vector<MountEntry> mounts = read_mount_table(paths_.mountinfo_path);
    const bool mounted = mount_at(mounts, mountpoint).has_value();
