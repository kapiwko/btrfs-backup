...
BrowseDirectoryPage BrowseSessionService::list_directory_page(
    ...
    const std::string after_name = continuation_tokens_.directory_cursor(
        session->second.id,
        relative_path,
        continuation_token
    );
    ...
    if (!page.continuation_token.empty())
        page.continuation_token = continuation_tokens_.directory_token(
            session->second.id,
            relative_path,
...
PreviousVersionsPage BrowseSessionService::list_previous_versions(
    ...
    const std::size_t offset = continuation_tokens_.previous_versions_offset(
        session->second.id,
        profile_id,
        source_id,
        relative_path,
    ...
    if (page.has_more) {
        ...
        page.continuation_token = continuation_tokens_.previous_versions_token(
            session->second.id,
            profile_id,
            source_id,
            relative_path,
