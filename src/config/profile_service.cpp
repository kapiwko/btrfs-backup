...
Profile validate_profile_file(const fs::path& file) {
    return profile_from_json(load_json_file(file));
}
...
void render_profile(const fs::path& file, const fs::path& output_dir) {
    ...
    render_tree(validate_profile_file(file), normalized);
}
